...
if __name__ == "__main__": #wrapping allows multiple workers on dataloader
    ...
    current_path = os.path.join("results/DAG","epochs_training_log.txt")
    ...
    # Find all cycles
    cycles = find_cycles(antarctica)

    # Print cycles and accumulate species appearances
    species_counter = Counter()

    for num, cycle in enumerate(cycles):
        print(f"Num cycle {num}")
        species_names = antarctica.vs[cycle]["name"]
        print(species_names)
        species_counter.update(set(species_names))

    # Calculate percentage presence in cycles
    total_cycles = len(cycles)

    print("\nSpecies participation in cycles (percentage):")
    for species, count in species_counter.items():
        percentage = (count / total_cycles) * 100
        print(f"{species}: {percentage:.2f}%")
    
    plot_trophic_web_by_module(antarctica)
    ...
    for extinction in range(0, extinction_steps): #range(extinction_steps)
        ...
        for test in tests:
            ...
            models_dir = f"models/DAG/epoch_training/{test}/"
            ...
            for epoch_bound in epoch_bounds:
                ...
                for trial in range(num_trials):
                    ...
                    current_result_dict = {}
                    current_result_dict["name"] = f"DAG__{test}__{int((extinction_steps - extinction)/extinction_steps * 100)}_perc_left__epoch_{epoch_bound}__trial_{trial}"
                    ...
                    print(f"On trial {trial} with {epoch_bound} epochs of test {test} with {int((extinction_steps - extinction)/extinction_steps * 100)} percent left of pop")
                    print("Updated Arctic num basal species: " + str(sum([1 for i in range(arctic.vcount()) if arctic.degree(i, mode = "in") == 0 and arctic.degree(i, mode = "out") > 0])))
                    print("Updated Arctic num apex species: " + str(sum([1 for i in range(arctic.vcount()) if arctic.degree(i, mode = "in") > 0 and arctic.degree(i, mode = "out") == 0])))
                    ...
                    logging.info(f"On trial {trial} with {epoch_bound} epochs of test {test} with {int((extinction_steps - extinction)/extinction_steps * 100)} percent left of pop")

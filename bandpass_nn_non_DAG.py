...
if __name__ == "__main__": #wrapping allows multiple workers on dataloader
    ...
    current_path = os.path.join("results/non-DAG","training_log.txt")
    ...
    original_vcount = antarctica.vcount()
    extinction_steps = 10
    num_trials = 5
    num_tests = len(tests)
    ...
    max_freq = 1000  # Maximum frequency for random sinusoids
    bandpass_low = 50  # Bandpass filter lower cutoff
    bandpass_high = 250  # Bandpass filter upper cutoff

    # Generate a random mixture of sinusoids
    np.random.seed(42)
    time = np.arange(num_samples) / sample_rate
    frequencies = np.random.uniform(0, max_freq, size=20)
    amplitudes = np.random.uniform(0.1, 1.0, size=20)

    # Sum all sinusoids to form the input signal
    input_signal = np.zeros(num_samples)
    for f, a in zip(frequencies, amplitudes):
        input_signal += a * np.sin(2 * np.pi * f * time)

    # Design band-pass filter
    nyquist = sample_rate / 2.0
    b, a = butter(4, [bandpass_low / nyquist, bandpass_high / nyquist], btype='band')

    # Apply the band-pass filter to get the target signal
    target_signal = filtfilt(b, a, input_signal)

    # Plot the input and target signals
    plt.figure(figsize=(15, 6))
    plt.subplot(2, 1, 1)
    plt.plot(time[:1000], input_signal[:1000])
    plt.title('Input Signal (Mixed Sinusoids)')
    plt.subplot(2, 1, 2)
    plt.plot(time[:1000], target_signal[:1000])
    plt.title(f'Target Signal (Band-pass {bandpass_low}-{bandpass_high} Hz)')
    plt.show()
    ...
    for extinction in range(extinction_steps):
        ...
        models_dir = f"models/non-DAG/{test}/"
        ...
        for trial in range(num_trials):
            ...
            current_result_dict = {}
            current_result_dict["name"] = f"non_DAG__{test}__{extinction_steps - extinction}0_perc_left__trial_{trial}"
            ...
            print(f"On trial {trial} of test {test} with {extinction_steps - extinction} percent left of pop")
            print("Updated antarctica num basal species: " + str(sum([1 for i in range(antarctica.vcount()) if antarctica.degree(i, mode = "in") == 0 and antarctica.degree(i, mode = "out") > 0])))
            print("Updated antarctica num apex species: " + str(sum([1 for i in range(antarctica.vcount()) if antarctica.degree(i, mode = "in") > 0 and antarctica.degree(i, mode = "out") == 0])))

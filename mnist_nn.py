...
if __name__ == "__main__": #wrapping allows multiple workers on dataloader
    ...
    current_path = os.path.join("results/DAG","mnist_training_log.txt")
    ...
    arctic_base = arctic.copy()

    arctic_double= horzcat_graph(arctic)
    arctic = vertcat_graph(arctic_double, arctic )
    

    arctic_double= horzcat_graph(arctic)
    arctic = vertcat_graph(arctic_double, arctic_base)
    ...
    extinction_steps = 100
    num_trials = 5    


    print(original_vcount)
    for extinction in range(80, extinction_steps, 2):
...
##########
        ...
        for trial in range(num_trials):
            ...
            current_result_dict = {}
            current_result_dict["name"] = f"DAG__{test}__{extinction_steps - extinction}_perc_left__trial_{trial}"
            ...
            X = torch.cat([train_dataset.data, test_dataset.data], dim=0).unsqueeze(1).float() / 255.0
            X = X.view(X.size(0), -1)
